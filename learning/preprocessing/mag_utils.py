...
class DeviceCalibration:
    ...
    def from_trial(cls, trial):
        # 此处更改为aura的数据类型应该是可靠的
        calib = cls()
        # filename = os.path.join(DATA_ROOT, "ceres", f"calibrate__{trial}.csv")
        filename = "/home/gaofei/BiRing/Data/Calibration.txt"
...
def get_field_new(ring_pos, ring_q, base_sensor_offset, sensor_offset, sensor_rot_offset, base_sensor_rot_offset,
...
def compute_sensor_new(field, global_gain,per_channel_gain,kxy,kxz,kyz,noise,gain,bias):
    # 对应的还没找到先选择理解为auraring中的读数
...
if __name__ == "__main__":
    data = np.loadtxt("/home/gaofei/Downloads/6dof_9axis_1.txt")
    print(data)
