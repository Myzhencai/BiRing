...
WRIST_OPTI, WRIST_OPTI_POS, WRIST_OPTI_ROT, WRIST_ROT = make_all_names('wrist')
FINGER_OPTI, FINGER_OPTI_POS, FINGER_OPTI_ROT, FINGER_ROT = make_all_names('finger')
PALM_OPTI, PALM_OPTI_POS, PALM_OPTI_ROT, PALM_ROT = make_all_names('palm')
...
def load_opti_metadata(key):
    ...
    include_palm = entry.use_palm.iloc[0]
    include_markers = entry.use_markers.iloc[0]
    rigid_body_calib = entry.rigid_body_calib.iloc[0]

    return file, include_palm, include_markers, rigid_body_calib

def load_opti_data(key):
    file, include_palm, include_markers, rigid_body_calib = load_opti_metadata(key)
    if os.path.exists(file+".pkl"):
        df = pickle.load(open(file+".pkl", 'rb'))
        return df, include_palm, include_markers, rigid_body_calib
    ...
    columns = ['frame_wrist'] + [f"wrist_{x}" for x in OPTI_NAMES] + \
              [f"finger_{x}" for x in OPTI_NAMES]
    if include_palm:
        columns += [f"palm_{x}" for x in OPTI_NAMES]
    if include_markers:
        columns += ['marker_wrist', 'marker_finger']
        if include_palm:
            columns += ['marker_palm']
    temp = []
    ...
    with open(file, 'rb') as f:
        ...
        for i, line in enumerate(f):
            ...
            if include_palm:
                a = [m['frame_num'], *[i for i in (extract_body(m['wrist']) + extract_body(m['finger']) + extract_body(m['palm']))]]
            else:
                a = [m['frame_num'], *[i for i in (extract_body(m['wrist']) + extract_body(m['finger']))]]
            if include_markers:
                a += m['markers'][b'wrist'], m['markers'][b'finger']
                if include_palm:
                    a += m['markers'][b'palm']
            temp.append(a)
    progress(1)
    ...
    # wy = df.wrist_y.values.copy()
    # wz = df.wrist_z.values.copy()
    #
    # df.wrist_y = wz
    # df.wrist_z = -wy
    #
    # wqy = df.wrist_qy.values.copy()
    # wqz = df.wrist_qz.values.copy()
    #
    # df.wrist_qy = wqz
    # df.wrist_qz = -wqy
    #
    # fy = df.finger_y.values.copy()
    # fz = df.finger_z.values.copy()
    #
    # df.finger_y = fz
    # df.finger_z = -fy
...
def load_resampled_data(key, variant="", raw=False):
    # file = get_resampled_file(key, variant)
    file = "/home/gaofei/BiRing/Data/processed__t1.txt"
    # with open(file.replace('.csv', '.pkl'), 'rb') as f:
    with open(file, 'rb') as f:
        all_data = np.loadtxt(file)
    ...
    if raw:
        ...
        mag_data = all_data[:, 0:3]
        mag_data = mag_data[:, [0, 2, 1]]
        pos = all_data[:, 3:6] * 1000
        rot = all_data[:, 6:10]
...
def save_predictions(key, dataset, preds, dof):
    file = os.path.join(PREDICTIONS_DIR, "predictions_%s_%s_%s.txt" % (key, dataset, dof))
    np.savetxt(file, preds)


def save_pos_data(key, dataset, preds, dof):
    file = os.path.join(PREDICTIONS_DIR, "pos_%s_%s_%s.txt" % (key, dataset, dof))
    np.savetxt(file, preds)


def save_mode(model, key, dataset, calib_on, n_dof):
    file = os.path.join(MODEL_DIR, "model_%s_%s_calib%s_dof%s" % (key, dataset, calib_on, n_dof))
...
def load_pos(key, dataset):
    file = os.path.join(PREDICTIONS_DIR, "pos_%s_%s.txt" % (key, dataset))
    return np.loadtxt(file)
